...
COVER = 'images/Figure_1.png'

class Study_Fatigue_detecting(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        sbSizer1 = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, u"学习状态监控"), wx.VERTICAL)
        sbSizer2 = wx.StaticBoxSizer(wx.StaticBox(sbSizer1.GetStaticBox(), wx.ID_ANY, u"基础操作"), wx.VERTICAL)
        gSizer1 = wx.GridSizer(0, 2, 0, 8)
        self.m_choice1 = wx.Button(sbSizer2.GetStaticBox(), wx.ID_ANY, u"加载摄像头", wx.DefaultPosition, wx.Size(120, 60),
                                   0)
        gSizer1.Add(self.m_choice1, 0, wx.ALL, 5)
        self.camera_button1 = wx.Button(sbSizer2.GetStaticBox(), wx.ID_ANY, u"开始监控", wx.DefaultPosition,
                                        wx.Size(120, 60), 0)
        ...
        self.off_button4 = wx.Button(sbSizer2.GetStaticBox(), wx.ID_ANY, u"退出监控", wx.DefaultPosition, wx.Size(120, 60),
                                     0)
        gSizer1.Add(self.off_button4, 0, wx.ALL, 5)
        sbSizer2.Add(gSizer1, 1, wx.EXPAND, 5)
        sbSizer1.Add(sbSizer2, 2, wx.EXPAND, 5)
        sbSizer6 = wx.StaticBoxSizer(wx.StaticBox(sbSizer1.GetStaticBox(), wx.ID_ANY, u"学习状态输出"), wx.VERTICAL)
        ...
        self.Layout()
        self.Centre(wx.BOTH)
        # font = wx.Font(12, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, "Microsoft YaHei")
        # self.SetFont(font)
        ...
        # 设置窗口标题的图标
        self.icon = wx.Icon('images/555.ico', wx.BITMAP_TYPE_ICO)
    ...
    def _learning_face(self, event):
        ...
        if self.cap.isOpened() == True:  # 返回true/false 检查初始化是否成功
            ...
            engine.Speak('打开摄像头成功，开始为您监控学习状态，祝您学习愉快')
        else:
            engine = pyttsx3.init()
            content = "打开摄像头失败，请重试"
            print('准备开始语音播报...')
        ...
        while (self.cap.isOpened()):
            ...
            if (len(faces) != 0):
                ...
                if self.oCOUNTER >= self.OUT_AR_CONSEC_FRAMES_check:
                    self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"检测到分神行为!!!\n")
                    self.oCOUNTER = 0

            # 判断疲劳程度
            if self.score >= 30 and self.score <= 55:
                cv2.putText(im_rd, "轻度疲劳", (350, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            if self.score > 55 and self.score <= 75:
                cv2.putText(im_rd, "中度疲劳", (350, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            if self.score > 75:
                cv2.putText(im_rd, "重度疲劳", (350, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

            T = time.time() - start
            fps = 1 / T  # 实时在视频上显示fps
            fps_txt = 'fps:%.2f' % (fps)
            # cv2.putText(im_rd, "眨眼次数: {}".format(self.TOTAL), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            # cv2.putText(im_rd, "点头次数: {}".format(self.hTOTAL), (450, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            # cv2.putText(im_rd, "哈欠次数: {}".format(self.mTOTAL), (210, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            # cv2.putText(im_rd, "眨眼频率:{}".format(self.frequency), (10, 65), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            # cv2.putText(im_rd, fps_txt, (450, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
            # cv2.putText(im_rd, "疲劳程度:{}".format(self.score), (10, 170), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            # cv2.putText(im_rd, "哈欠频率:{}".format(self.yfrequency), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            # cv2.putText(im_rd, "点头频率:{}".format(self.hfrequency), (10, 135), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
        ...
        for i in range(500):
            ...
            if self.score >= 30 and self.score <= 55:
                self.m_textCtrl3.AppendText(
                    time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"提醒：检测到轻度疲劳，建议休息一下！\n准备开始语音播报\n")
                pythoncom.CoInitialize()
                engine = client.Dispatch("SAPI.SpVoice")
                engine.Speak('提醒提醒，检测到您已进入轻度疲劳状态，建议休息一下，保持学习效率')
                # 语音播报内容
                # self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"提醒：检测到轻度疲劳，建议休息一下！\n准备开始语音播报\n")
            if self.score > 55 and self.score <= 75:
                # 语音播报内容
                self.m_textCtrl3.AppendText(
                    time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"警告：检测到中度疲劳，请立即休息！\n准备开始语音播报\n")
                pythoncom.CoInitialize()
                engine = client.Dispatch("SAPI.SpVoice")
                engine.Speak('警告警告，检测到您已进入中度疲劳状态，请立即休息，否则学习效率将大幅下降')
            if self.score > 75:
                # 语音播报内容
                self.m_textCtrl3.AppendText(
                    time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"严重警告：检测到重度疲劳，请立即停止学习！\n准备开始语音播报\n")
                pythoncom.CoInitialize()
                engine = client.Dispatch("SAPI.SpVoice")
                engine.Speak('严重警告，检测到您已进入重度疲劳状态，请立即停止学习，休息后再继续')
    ...
    def OnClose(self, evt):
        ...
        if (dlg.ShowModal() == wx.ID_YES):
            ...
            pythoncom.CoInitialize()
            engine = client.Dispatch("SAPI.SpVoice")
            engine.Speak('再见，欢迎您再次使用，祝您学习进步')
            print("监控结束，成功退出程序!!!")
            sats2.page_draggable_layout()

    def exit(self, evt):
        dlg = wx.MessageDialog(None, u'确定要退出监控吗？', u'操作提示', wx.YES_NO | wx.ICON_QUESTION)
        if (dlg.ShowModal() == wx.ID_YES):
            self.Destroy()
            pythoncom.CoInitialize()
            engine = client.Dispatch("SAPI.SpVoice")
    ...
    def prepare(self, evt):
        self.m_textCtrl3.AppendText(u"加载摄像头成功!!!\n")
        pythoncom.CoInitialize()
        engine = client.Dispatch("SAPI.SpVoice")
        engine.Speak('初始化摄像头成功')

class main_app(wx.App):
    # OnInit 方法在主事件循环开始前被wxPython系统调用，是wxpython独有的
    def OnInit(self):
        self.frame = Study_Fatigue_detecting(parent=None, title="学生学习疲劳检测系统")
